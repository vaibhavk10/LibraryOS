...
export default function Login() {
  ...
  const { login, verifyCode: verifySignupCode, loading, error } = useAuthContext();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (user) {
      if (user.emailVerified === false && user.role !== 'member') {
        toast.info('Please verify your email first, then sign in again.');
        return;
      }
  ...
  const handleVerify = async () => {
    if (!email.trim() || !verifyCode.trim()) {
      toast.info('Enter your email and verification code.');
      return;
    }
      ...
      >
        <div className="text-center mb-8">
          ...
          {searchParams.get('verify') === '1' && (
            <p className="text-xs mt-2 text-[hsl(45,90%,65%)]">
              Verify your email before accessing dashboard routes.
            </p>
          )}
        </div>
        ...
        <div className="bg-[hsl(220,18%,10%)] border border-[hsl(220,15%,18%)] rounded-2xl p-8">
          ...
          <div className="mt-4 p-3 bg-[hsl(220,18%,12%)] border border-[hsl(220,15%,20%)] rounded-lg">
            <button
              type="button"
              onClick={() => setShowVerify((s) => !s)}
              className="text-xs text-[hsl(45,90%,65%)] hover:text-[hsl(45,90%,75%)]"
            >
              {showVerify ? 'Hide verification code form' : 'Got email verification code? Verify here'}
            </button>
            {showVerify && (
              <div className="mt-3 space-y-2">
                <input
                  ...
                  className="w-full px-3 py-2 bg-[hsl(220,18%,13%)] border border-[hsl(220,15%,22%)] text-white placeholder-[hsl(220,10%,38%)] rounded-lg text-sm"
                />
                <button
                  type="button"
                  onClick={handleVerify}
                  disabled={loading}
                  className="px-4 py-2 text-xs font-medium bg-[hsl(45,90%,55%)] text-[hsl(220,20%,8%)] rounded-lg disabled:opacity-60"
                >
                  Verify Code
                </button>
              </div>
          ...
          <p className="text-center text-xs text-[hsl(220,10%,45%)] mt-6">
            ...
            <Link to="/signup" className="text-[hsl(45,90%,55%)] hover:text-[hsl(45,90%,65%)] font-medium transition-colors">
              Create account
            </Link>
          </p>
        </div>
